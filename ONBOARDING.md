...
- `lmstudio` — local server at `http://127.0.0.1:1234` (override with `LMSTUDIO_BASE_URL`; a trailing `/v1` on the env var is accepted for backward compatibility). The sidecar registers the provider on first use with a dummy API key, since LM Studio accepts any. The per-agent context window is auto-detected from LM Studio's native `/api/v0/models` endpoint (live `loaded_context_length`, falling back to `max_context_length` for models that aren't loaded yet), surfaced read-only in the spawn dialog, then persisted on the `agents.context_window` column and forwarded to the sidecar as `CreateSessionCommand.contextWindow`. If the native endpoint isn't available (older LM Studio, model missing from the list), the sidecar's 32k default is used instead.

Model discovery lives in [`src-tauri/src/models.rs`](./src-tauri/src/models.rs) and is exposed as `get_models` and `get_provider_auth_status` Tauri commands. For `openrouter`, `get_models` hits the provider's `/models` endpoint directly. For `lmstudio`, it prefers the native `/api/v0/models` endpoint (which carries `loaded_context_length` / `max_context_length` per entry, surfaced on `ModelInfo.contextWindow`) and falls back to the OpenAI-compatible `/v1/models` path on any failure. The LM Studio arm returns `Err(...)` when neither endpoint responds so the UI can show a distinct "provider unreachable" state.
