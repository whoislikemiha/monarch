...
- `lmstudio` — local server at `http://127.0.0.1:1234` (override with `LMSTUDIO_BASE_URL`; a trailing `/v1` on the env var is accepted for backward compatibility). The sidecar registers the provider on first use with a dummy API key, since LM Studio accepts any. The spawn dialog still asks for a per-agent context window — persisted on the `agents.context_window` column and forwarded to the sidecar as `CreateSessionCommand.contextWindow` with a 32k fallback — but it pre-fills from LM Studio's native `/api/v0/models` endpoint when available (live `loaded_context_length`, falling back to `max_context_length` for models that aren't loaded yet).
