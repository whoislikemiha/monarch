...
- `CLAUDE.md` was updated with the new files, schema table, event channel, and config file. `ONBOARDING.md` was intentionally left for a follow-up — its data-model / protocol sections need a broader refresh that the Slice 1 diff alone doesn't warrant.
...
- **Exact id pairing for DB linkage; ordinal pairing for UI rendering.** Persistence uses the sidecar-minted `classification_id` so the `message_id` FK backfill is exact. In practice the classifier (~1.5 s on Haiku) usually lands AFTER Pi's user `message_end` (instant), so the inline backfill in `SaveAssistantMessage` would normally hit 0 rows. A `PersistContext` threaded through the single-consumer loop stashes the pending `(classification_id → message_id)` pair when the UPDATE finds nothing, and `SaveClassification` consumes the stash on INSERT — both orderings resolve. The frontend deliberately avoids threading the id through `LiveAgentState`; instead it maps live classification events to user-message ordinals in FIFO order and rehydrates historical rows by `message_id` when a session is rebound.
