...
- **Schema evolves via `ALTER TABLE` migrations** — `db::init_schema` applies idempotent `ALTER TABLE` / `CREATE TABLE IF NOT EXISTS` blocks at the end of init. Never rewrite the base `CREATE TABLE` for columns added post-launch — add a new migration block. Current post-launch columns: `sessions.parent_session_id`, `agents.project_id`, `agents.context_window`, `agents.archived_at`, `agents.avatar_type`, `agents.avatar_path`, `agents.current_objective_id`, `messages.duration_ms`, `messages.objective_id`, plus nested objective-event columns `objective_events.parent_event_id`, `objective_events.author`, `objective_events.surface_override`, `objective_events.payload_schema_version`, `objective_events.plan_item_id`, and rich objective columns `objective_nodes.scope`, `objective_nodes.current_direction`, `objective_nodes.rationale`, `objective_nodes.fork_parent_id`, plus P1 campaign columns `objective_nodes.kind`, `projects.root_objective_id`. Post-launch tables: `projects`, `agent_templates`, `ui_state`, `agent_stats`, `agent_tool_usage`, `message_attachments`, `objective_nodes`, `objective_events`, `agent_working_memory`, `objective_plan_items`, `objective_refs`, `objective_reports`, `classifications`.
...
- **Objectives are orthogonal to sessions** (MON-83) — an objective can span multiple sessions, a session can span multiple objectives. Aggregation key for "what happened on this objective" is `objective_id`, not `session_id`. Objectives are Monarch's canonical work object; external trackers can be attachments/refs, not the authority.
- **Campaign root is a typed node** (P1, roadmap-v2) — each project has exactly one *campaign*: an `objective_nodes` row with `kind='campaign'`, `parent_id=NULL`, `root_id=self`, linked from `projects.root_objective_id`. A never-closed container, not work — never assign/grade/close/report it; `WHERE kind='objective'` selects real work. Created on project detect via `ensure_campaign_root_internal` (idempotent). Meaningful turns branch UNDER it (`auto_create_current_objective_internal` sets `parent_id = root_id = campaign root`, not a fresh per-turn root); **project-less agents stay ephemeral** (no project → no campaign → no auto-objective). The timeline groups by `root_id`; `db_get_campaign_root_for_agent` resolves the capture target.
- **Objective / plan / timeline are distinct** (roadmap P4/P4b) — objective = what/why, durable execution plan = intended how, timeline coherent actions = what actually happened. Don't treat coherent actions as plan items, and don't store future plan fields until P4b has writers.
- **Objective refs are references, not authority** (roadmap P5) — `objective_refs` links objectives to Linear/GitHub/files/URLs/artifacts. The objective remains canonical; external trackers are context and evidence.
