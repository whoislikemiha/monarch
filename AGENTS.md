...
- **Executor narration is semantic, not chatty** (roadmap P4) — narration tools should produce `coherent_action`, `executor_action_outcome`, and `executor_decision` events. Hide the narration tool calls themselves from timeline tool-call rows. Do not persist raw model thinking as timeline content; if rationale matters, record an explicit decision.
