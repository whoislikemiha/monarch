...
pub(super) fn inner_event_tag(event: &InnerEvent) -> &'static str {
    match event {
        ...
        InnerEvent::ExecutorPaused { .. } => "executor_paused",
        InnerEvent::ExecutorResumed { .. } => "executor_resumed",
        InnerEvent::ExecutorStopped { .. } => "executor_stopped",
        InnerEvent::ChatObservation { .. } => "chat_observation",
