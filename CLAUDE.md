...
- **Narration is objective-free and auto-harvested** (MON-124) — when an assistant turn contains process text followed by real (non-meta) tool calls, `harvest_narration_intent` (agent/persist/messages.rs) turns the trailing sentence into a `coherent_action` automatically; explicit `set_current_action`/`complete_action` always win. Narration/tool events with no current objective land on the agent's **scratch objective** (`scratch-{agent_id}`, `ensure_scratch_objective_internal`, created silently, never closed/graded/reported) — so unscoped work is durable, not dropped. The shadow's system prompt instructs one-sentence process narration before each tool batch.
