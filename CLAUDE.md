...
| Sidecar  | `sidecar/src/chat-tools.ts`                           | Chat-shadow tool set: recall/search, hand_to_executor, pause/resume/stop (MON-128) |
| Sidecar  | `sidecar/src/pause-gate.ts`                           | Executor pause gate — extension tool_call handler parks at tool boundaries (MON-128) |
...
- **Event channels** — `agent-state-{id}` (Rust-assembled EXECUTOR snapshots, canonical), `agent-chat-state-{id}` (chat-organ snapshots, independent stateVersion stream — MON-128), `agent-event-{id}` (out-of-band signals only), `agent-exit-{id}`, `agent-chat-exit-{id}`, `agent-stderr-{id}`, `agent-classification-{id}` (MON-82 per-turn classifier output).
...
- **One shadow, two organs, two Pi sessions** (MON-128, P3) — every agent can host an **executor** session (full toolset, acts on the world, narrates the timeline) and a **chat-shadow** session (read/direct/control only: read/grep/find/ls + recall_actions/memory_search/surface_observation/hand_to_executor/pause_resume_stop — NO bash, NO write). `sessions.role` ('executor' default | 'chat') marks the organ; the sidecar keys sessions by `(agentId, role)` and every session-addressed command/event carries an optional `sessionRole` (absent = executor). Chat panes send via `chat_prompt` (lazy-spawns the chat session from the executor's create config); the chat organ **persists dialogue only** — no timeline rows, no keeper triggers, no classifier, no turn/tool stats. Work crosses to the executor ONLY via `hand_to_executor`: a no-payload tool — Rust injects the **verbatim** conversation slice since the watermark (`sessions.last_handoff_message_id`), records `dispatched_to_executor`, and never paraphrases the captain's words. Role-less `destroy_session` tears down both organs (kill path); sidecar crash recovery replays executors only (chat respawns lazily).
- **Pause gate sharp edge** (MON-128) — the executor pause is an extension `tool_call` handler that parks at tool boundaries. Pi does NOT propagate abort into parked handlers, so **`abort()` hangs while a call is parked**: every abort/stop/destroy path MUST release the gate first (see `pause-gate.ts`; verified in `sidecar/src/spike-two-sessions.ts`). Control events land as `paused_by_chat`/`resumed_by_chat`/`stopped_by_chat` (or `*_by_captain`) timeline rows and flip `executor_paused` on live state.
