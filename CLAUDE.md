...
- **Narration cadence is enforced by the harness, not the prompt** (MON-130) — `sidecar/src/narration-nudge.ts` composes onto Pi's `afterToolCall` hook and appends a `<system-reminder>` to tool results after 6 consecutive non-meta tool calls without `set_current_action`/`complete_action` (re-nudging every 5). Meta tools reset the counter. Don't move this into the system prompt — prompt-only narration drifts after the first action.
- **Timeline previews are expandable, full data comes from `messages`** (MON-130) — `objective_events` tool payloads keep the 500-char `args_preview`; clicking a tool row fetches the FULL args/result via `db_get_tool_call_detail(tool_call_id)` (recovered from the assistant `toolCall` block + paired `toolResult` row in `messages`). Don't fatten event payloads to show more text — fetch on demand.
- **Memory keeper is on by default** (MON-130) — `memory.toml` absent or missing `[keeper]` resolves to `anthropic/claude-haiku-4-5`; only a top-level `enabled = false` disables the Curator. Gate Curator work on `ResolvedMemoryConfig.enabled`, never on `keeper.is_some()` (it's always `Some` after resolve).
...
- **Avatars default to the provider logo** — `avatar_type` NULL means "auto": `src/lib/ui/Avatar.svelte` renders the agent's provider logo (bundled at `static/avatars/providers/{provider}.svg`, map in `src/lib/avatar/providerLogos.ts`; monogram only for unknown providers). `avatar_type = "image"` + `avatar_path` is an explicit override — a provider-logo preset or an upload under `~/.config/monarch/avatars/`. The Rive system and the silhouette preset were removed (stale rows are cleared by startup migrations).
